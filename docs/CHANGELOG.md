...
## [0.3.8.7] - 2025-07-20 - VYLEPŠENÍ CENOVÉHO MODELU, MONETIZACE, LOGOVÁNÍ A MAPOVÝCH SLUŽEB

### Nové funkce
- Implementován vylepšený cenový model pro LLM API s marží pro aplikaci
- Přidáno sledování zisku z marže pro lepší přehled o nákladech a výnosech
- Vytvořena podrobná dokumentace cenového modelu v souboru PRICING_MODEL.md
- Přidána super-dokumentace jako centrální rozcestník pro všechny dokumentační soubory
- Implementována transparentní vizualizace marže v uživatelském rozhraní
- Přidán komplexní systém logování komunikace s LLM API
- Implementováno ukládání logů do souboru a Supabase pro analýzu a ladění
- Přidána podpora pro různé mapové poskytovatele se zaměřením na střední Evropu
- Implementována integrace s Mapy.cz, OpenRouteService, Windy a FreemapSlovakia
- Vytvořen systém pro automatický výběr nejlepšího mapového poskytovatele podle země

### Vylepšení
- Optimalizován výpočet ceny požadavků na LLM API s přesnějším odhadem tokenů
- Vylepšeno sledování využití API s detailnějšími statistikami
- Aktualizovány ceny všech podporovaných modelů LLM API
- Přidána podpora pro sledování základních nákladů API a marže aplikace
- Vylepšena dokumentace monetizačního modelu s aktuálními informacemi
- Přidány statistiky využití LLM API s detailními informacemi o nákladech a výkonu
- Implementováno sledování latence a výkonu LLM API pro optimalizaci
- Vylepšeno vyhledávání míst s podporou lokálních poskytovatelů pro přesnější výsledky
- Přidána podpora pro turistické a cyklistické mapy ve střední Evropě
- Implementována podpora pro offline režim s možností stažení mapových dlaždic
- Přidána integrace s předpovědí počasí pomocí Windy API

### Opravy
- Opraveno zobrazování zbývajícího kreditu při použití LLM API
- Vylepšena přesnost odhadu ceny požadavků pro různé jazyky a formáty
- Opraveno resetování statistik využití API po 30 dnech

## [0.3.8.6] - 2025-07-15 - VYLEPŠENÍ DOKUMENTACE A STABILIZACE INTEGRACE

### Nové funkce
- Přidána kompletní dokumentace mapových funkcí a navigace mezi stránkami
- Implementována podrobná dokumentace integrace Auth0 a Supabase
- Přidán přehled klíčových funkcí aplikace s detailním popisem
- Vytvořen centrální rozcestník dokumentace pro snadnější orientaci
- Přidána dokumentace pro vývojáře s popisem implementace funkcí
- Implementována podpora pro offline dokumentaci v aplikaci

### Vylepšení
- Stabilizována integrace Auth0 a Supabase s lepší synchronizací dat
- Optimalizován proces přihlašování pro rychlejší odezvu
- Vylepšena detekce chyb při autentizaci s podrobnějšími chybovými zprávami
- Přidána podpora pro automatické obnovení tokenů při vypršení platnosti
- Vylepšena bezpečnost aplikace s lepším ošetřením chybových stavů
- Optimalizováno načítání aplikace pro rychlejší start
- Vylepšena kompatibilita s různými prohlížeči a zařízeními
- Přidána podpora pro PWA (Progressive Web App) s offline funkcionalitou
- Vylepšena dokumentace API s příklady použití

### Opravy
- Opraveno přesměrování po odhlášení na produkčním serveru
- Vyřešen problém s nesprávným zobrazením uživatelského profilu po přihlášení
- Opravena synchronizace dat mezi Auth0 a Supabase při prvním přihlášení
- Vyřešeny problémy s CORS při komunikaci s API
- Opraveno ukládání uživatelských preferencí v Supabase
- Vylepšena odolnost aplikace proti výpadkům připojení
- Opraveno zobrazení chybových zpráv při selhání autentizace
- Vyřešen problém s duplikací uživatelských dat při synchronizaci
