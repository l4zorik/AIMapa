...
const BusinessMarkers = {
    ...
    addStyles() {
        ...
        styleElement.textContent = `
            ...
            .business-filter-option:hover {
                background: #e9ecef;
            }
            
            .business-filter-option.active {
                background: var(--option-color);
                color: white;
            }
            
            .business-filter-option-icon {
                font-size: 14px;
            }
    ...
    addBusinessesToMap() {
        ...
        // Přidání filtru firem
        this.addBusinessFilter();
    },
    ...
    // Přidání filtru firem
    addBusinessFilter() {
        // Kontrola, zda již existuje filtr
        if (document.getElementById('business-filter')) {
            return;
        }
        
        // Vytvoření filtru
        const filterContainer = document.createElement('div');
        filterContainer.id = 'business-filter';
        filterContainer.className = 'business-filter';
        
        // Vytvoření obsahu filtru
        let filterOptionsHtml = '';
        Object.entries(this.businessTypes).forEach(([type, data]) => {
            filterOptionsHtml += `
                <div class="business-filter-option active" data-type="${type}" style="--option-color: ${data.color}">
                    <span class="business-filter-option-icon">${data.icon}</span>
                    <span>${data.name}</span>
                </div>
            `;
        });
        ...
        filterContainer.innerHTML = `
            <div class="business-filter-header">
                <h4>Filtr firem</h4>
                <button class="business-filter-toggle" id="business-filter-toggle">
                    <i class="fas fa-chevron-up"></i>
                </button>
            </div>
            <div class="business-filter-options" id="business-filter-options">
                <div class="business-filter-option active" data-type="all" style="--option-color: #34495e">
                    <span class="business-filter-option-icon">🏙️</span>
                    <span>Vše</span>
                </div>
                ${filterOptionsHtml}
            </div>
        ...
        // Přidání filtru do stránky
        document.body.appendChild(filterContainer);
        
        // Přidání event listenerů pro filtr
        this.setupFilterEventListeners();
    },
    
    // Nastavení event listenerů pro filtr
    setupFilterEventListeners() {
        // Získání elementů
        const filterToggle = document.getElementById('business-filter-toggle');
        const filterOptions = document.getElementById('business-filter-options');
        const filterOptionElements = document.querySelectorAll('.business-filter-option');
        
        // Event listener pro zobrazení/skrytí filtru
        if (filterToggle && filterOptions) {
            filterToggle.addEventListener('click', () => {
                const isVisible = filterOptions.style.display !== 'none';
                filterOptions.style.display = isVisible ? 'none' : 'flex';
                filterToggle.innerHTML = isVisible ? '<i class="fas fa-chevron-down"></i>' : '<i class="fas fa-chevron-up"></i>';
            });
        }
        
        // Event listener pro filtrování firem
        if (filterOptionElements) {
            filterOptionElements.forEach(option => {
                option.addEventListener('click', () => {
                    const type = option.dataset.type;
                    
                    // Pokud je vybrána možnost "Vše", aktivujeme všechny možnosti
                    if (type === 'all') {
                        filterOptionElements.forEach(opt => {
                            opt.classList.add('active');
                        });
                        
                        // Zobrazení všech markerů
                        ...
                        // Získání všech aktivních typů
                        const activeTypes = [];
                        filterOptionElements.forEach(opt => {
                            if (opt.classList.contains('active') && opt.dataset.type !== 'all') {
                                activeTypes.push(opt.dataset.type);
                            }
                        });
                        
                        // Aktualizace stavu možnosti "Vše"
                        const allOption = document.querySelector('.business-filter-option[data-type="all"]');
                        if (allOption) {
                            if (activeTypes.length === Object.keys(this.businessTypes).length) {
                        ...
                        // Filtrování markerů
                        this.businessMarkers.forEach(item => {
                            if (activeTypes.includes(item.business.type)) {
                                item.marker.addTo(map);
                            } else {
