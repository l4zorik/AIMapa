...
function fixMap() {
    ...
    // Pokud není inicializována mapa, pokusíme se ji inicializovat
    if (!mapInitialized && leafletLoaded && mapElementExists) {
        try {
            console.log('Pokus o inicializaci mapy...');
            window.map = L.map('map', {
                zoomAnimation: true,
    ...
    // Pokud je mapa inicializována, ale nejsou viditelné dlaždice, pokusíme se o překreslení mapy
    if (window.map && !mapTilesLoaded) {
        console.log('Pokus o překreslení mapy...');
        window.map.invalidateSize();
    }
    
    // Pokud je mapa inicializována, ale není viditelná, pokusíme se ji zviditelnit
    if (window.map && !mapVisible) {
        console.log('Pokus o zviditelnění mapy...');
        const mapElement = document.getElementById('map');
        if (mapElement) {
            mapElement.style.display = 'block';
            mapElement.style.visibility = 'visible';
